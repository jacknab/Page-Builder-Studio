...
function buildSeoMeta(clientData, pageTitle) {
  ...
  const metaDescription = clientData.description
    ...
    : [
        `${clientData.businessName} is your local barbershop`,
        city ? `in ${city}` : null,
        "offering expert haircuts, fades, beard trims, and hot towel shaves.",
        "Walk-ins welcome.",
      ]
        .filter(Boolean)
        .join(" ");
  ...
  const keywords = [
    ...
    city ? `${city} barbershop` : null,
    city ? `barber ${city}` : null,
    city ? `haircut ${city}` : null,
    city ? `best barbershop ${city}` : null,
    city ? `fade ${city}` : null,
    city ? `beard trim ${city}` : null,
    city ? `walk-in barbershop ${city}` : null,
    city ? `hot towel shave ${city}` : null,
    "walk-in haircut",
    "hot towel shave",
